...
const SecondStep: React.FC<SecondStepProps> = ({ nextStep, data }) => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      first_name: Yup.string().required(t("secondStep.thisFieldRequired")),
      ...
      company_name: Yup.string().required(t("secondStep.thisFieldRequired")),
    }),
    onSubmit: (values) => {
      ...
      axios[data.registered ? "put" : "post"](url, values)
        ...
        .catch(() => {
          Toast.current = toast.error(t("success.error"), {
            position: "bottom-right",
            autoClose: 5000,
            hideProgressBar: false,
            closeOnClick: true,
  ...
  useEffect(() => {
    ...
    if (
      ...
      (data?.consultancyslots && data?.consultancyslots.length > 0)
    ) {
  ...
  const fetchWorkshops = async () => {
    ...
    axios
      ...
      .then((res) => {
        setWorkshops(res.data?.sort((a, b) => a.order - b.order) || []); 
    })
      .catch((err) => {
        console.log(err);
      })
  ...
  const fetchConsultation = async () => {
    ...
    axios
      ...
      .then((res) => {
        setConsultations(res.data?.sort((a, b) => a.order - b.order) || []);
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className={`bg-white flex items-center justify-center h-full w-screen py-20 gradient px-5 md:px-0 ${
        isRTL ? "rtl" : "ltr"
      }`}
    >
      <form
      ...
      >
        <h1 className="font-cairo text-[35px] font-bold text-black leading-normal text-center">
        ...
        <div className="w-full h-full flex flex-col gap-5 items-center">
          ...
          <div
            className="relative flex items-center gap-2 w-full font-cairo"
            style={{
          ...
          >
            ...
            />
            <label htmlFor="showWorkshops" className="text-black font-bold">
            ...
            <>
              <ul className="flex space-x-4 gradient2 p-2 shadow-lg rounded-2xl w-full">
                <li className="flex-1">
                  <button
                    className={`w-full h-12 text-black rounded-2xl text-xs md:text-lg font-cairo font-bold ${
                      selectedTab === 0 ? "bg-white" : "bg-transparent"
                ...
                <li className="flex-1">
                  <button
                    className={`w-full h-12 text-black rounded-2xl text-xs md:text-lg font-cairo font-bold ${
                      selectedTab === 1 ? "bg-white" : "bg-transparent"
                    } transition-colors ease-in-out duration-300`}
                ...
                <li className="flex-1">
                  <button
                    className={`w-full h-12 text-black rounded-2xl text-xs md:text-lg font-cairo font-bold ${
                      selectedTab === 2 ? "bg-white" : "bg-transparent"
                    } transition-colors ease-in-out duration-300`}
              ...
              <h1
                className="font-cairo text-base font-bold text-red-500"
                style={{
                  direction: isRTL ? "rtl" : "ltr",
                }}
              >
                {selectedTab == 0 && t("secondStep.priorityNoticeWorkshop")}
                {selectedTab == 1 &&
                  t("secondStep.priorityNoticeConsultations")}
                {selectedTab == 2 && t("secondStep.priorityNoticeInvesments")}
              </h1>
              <WorkshopList
                workshops={workshops}
          ...
          <Button
            ...
            label={
              data.registered
                ? t("secondStep.updateData")
                : t("secondStep.register")
            }
