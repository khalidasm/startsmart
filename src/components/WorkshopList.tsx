            ...
            return(
              <WorkshopItem
                ...
                isDisabled={
                  ...
                  !!formik.values["workshops"]?.find(
                    (wkShop) =>
                      workshops
                        .find((w) => w.id == wkShop) // get the workshop object to compare the time
                        .time_en?.split(":")[0] === workshop.time_en?.split(":")[0] &&
                        (workshops
                        .find((w) => w.id == wkShop).date_en)?.split(" ")[0] === (workshop.date_en)?.split(" ")[0] &&
                        wkShop != workshop.id
                  )
                }
                isRTL={isRTL}
    ...
    >
      <div className={`flex items-center gap-2`}>
        {fieldName !== "investments" && (
          ...
          >
            <input
              ...
              name={workshop.title}
              value={workshop.id}
              disabled={isDisabled || workshop.title_en?.toLowerCase().trim() === "coming soon"}
              checked={formik.values[fieldName].includes(workshop.id)}
              onChange={(e) => {
        ...
        <h1 className="text-black font-bold text-lg font-cairo">
          <span className="font-bold">{t("workshopList.workshopName")}</span>
      ...
      >
        <div className="flex flex-col gap-5">
          ...
          <div className="flex items-center gap-2">
            ...
            <p className="text-black">
              <span className="font-bold">{t("workshopList.instructor")}:</span>{" "}
          ...
          <div className="flex items-center gap-2">
            ...
            <p className="text-black">
              ...
              {isRTL ? workshop.time : workshop.time_en}
            </p>
          </div>
          {/* <div className="flex items-center gap-2">
